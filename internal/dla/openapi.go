		...
		"schemas": map[string]any{
			...
			"DLAEntryRequest": map[string]any{
				...
				"properties": map[string]any{
					...
					"amount":            map[string]any{"$ref": "#/components/schemas/DLAMoney"},
					"source_ref":        map[string]any{"type": "string", "description": "Optional idempotency/source reference. If supplied, it must start with manual:. If omitted, the server generates a manual source_ref and returns it."},
					"cash_account_code": map[string]any{"type": "string", "description": "Required for repayment entries; must resolve to a cash or bank asset account with matching currency."},
					"expense_category":  map[string]any{"type": "string", "description": "Required for expense-owed entries; use the target ledger expense account code with matching currency."},
				},
				"additionalProperties": false,
			},
