			...
			check: func(t *testing.T) {
				...
				want := map[string]struct {
					...
					"dla_overdrawn_bik": {
						...
						factQuery:    []string{"balance", "clearance_amount_minor_units", "director_id", "director_name", "status"},
						textTemplate: "{{ director_name }} loan account is {{ balance }} overdrawn. The Isle of Man has no UK-style s455 charge, but an interest-free loan can create a taxable benefit in kind - charge interest at the official rate or clear the director-specific balance.",
						ctaLabel:     "Review DLA",
						ctaAction:    "navigate:/dla?director={{ director_id }}",
					},
					"filing_deadline_window": {
						severity:     "amber",
						surfaces:     []string{"dashboard", "reports"},
						factQuery:    []string{"authority", "due_date", "days_until", "filing_name", "filing_status", "warn_window_days"},
